...
| [`EventTimeline`](../../packages/components/src/components/event-timeline/README.md)          | A bounded horizontal time window with proportionally positioned scheduled, fired, or upcoming events.                                                         | Vertical process history, dense activity, or execution state.                    | `Timeline` for a vertical event rail; `RunStepTimeline` for step state.                                        |
...
- **Vertical history versus horizontal schedule:** `Timeline` owns a readable
  event rail; `EventTimeline` owns proportional placement inside a bounded time
  range.
- **Activity versus diagnostics:** `Feed` is the user-facing activity stream
  and is the only member whose wrapper can opt into polite live announcements;
  `EventStreamViewer` is the operator-facing diagnostic console with stream
  controls and structured event inspection.
- **Static versus mutating content:** A chronological list is not implicitly a
  ...
  choose `EventStreamViewer` when follow-latest and operational stream state are
