...
| [`RunStepTimeline`](../../packages/components/src/components/run-step-timeline/README.md)     | The state of an asynchronous execution: pending/running/terminal statuses, durations, retries, progress, branches, compensation, and inline details.          | A flat timestamp log, a bounded schedule, an interactive wizard, or a dense operational log without structured step state. | `Timeline` for timestamp-first history; `Steps` for user-driven procedures; `EventStreamViewer` for dense operational logs. |
