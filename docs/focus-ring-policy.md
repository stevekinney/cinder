...
- `.cinder-collapsible__trigger:focus-visible` in `packages/components/src/components/collapsible/collapsible.css` — the trigger fills a rounded `.cinder-collapsible` root with `overflow: hidden`, so an outset ring would be clipped at the outer corners. The inset ring remains wholly inside the trigger while preserving its 44 px minimum touch target.
