...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Scoped `lint` (oxlint) over the touched-package closure          | `unit-tests.yaml` (`--filter='*' lint`, unconditional) and `main-green`                           |
...
| Scoped `test`/`test:changed` over the reverse-dependency closure | `unit-tests.yaml`'s scoped `test:changed` job and `main-green`'s full `test:changed` chunk matrix |
...
the inverse (`paths-ignore`) path set, whose jobs succeed immediately. For
any diff, at least one of the two workflows reports each required check name
— sometimes both (accepted; the skip job costs a few seconds of runner time)
— but never neither. `changeset-guard.yaml`'s path filter was removed the
