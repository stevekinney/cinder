...
| **main-green** (post-merge, nightly)                                        | Workspace `lint` + `lint:invariants`, workspace `typecheck`, generated artifact checks (`aggregator:check`, `components:check`), source audits (`check:placeholder-docs`, workflow validation, Svelte peer validation, `platform:audit`, `colors:audit`, `tokens:audit`), the FULL component suite through chunked `test:changed`, full component coverage via `test:coverage`, non-component workspace tests, the non-blocking memory canary, plus a single-version SvelteKit consumer **hydration smoke** (`validate:consumer:hydration-smoke`) that exercises the same Chromium launch as the release-path `validate:consumer` | The FULL consumer/tarball publish validation (`validate:consumer`, `package:weight:check`) — only the lighter hydration smoke runs here           | The authoritative source-validation gate. Push runs are keyed by SHA and are not cancelled by newer pushes because release waits for the same-SHA run before publishing. If this goes red, the class of bug PR CI's scoping missed is presumed to have leaked. The hydration smoke lives here (not just at release) so a Chromium-launch regression in `validate-consumers.ts` — which otherwise runs only in the release path — is caught before the same-SHA release can publish, rather than surfacing first as a blocked publish. |
