...
Disclosure and composite components use `useId()` (counter-based, deterministic within a session): Modal, Drawer, Popover, Tooltip, RadioGroup, CheckboxGroup, CommandPalette, ConfirmDialog, ColorPicker, Statistic, NavigationBar, SortableList, Dropdown, SideNavigationGroup, TreeItem.
