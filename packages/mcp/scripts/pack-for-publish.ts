...
/**
 * The source manifest declares `@lostgradient/cinder` via the `workspace:*` protocol (a
 * monorepo-only reference) — every other dependency must already be a real,
 * publishable range. `buildPublishedManifest` is responsible for rewriting the
...
export function assertSourceManifest(manifest: PackageManifest): void {
  ...
  if (JSON.stringify(otherDependencies).includes('workspace:')) {
    throw new Error(
      '@lostgradient/cinder-mcp dependencies must not use the workspace: protocol (other than @lostgradient/cinder)',
    );
  }
  if (manifest.dependencies?.['@lostgradient/cinder'] !== 'workspace:*') {
    throw new Error('@lostgradient/cinder-mcp must depend on @lostgradient/cinder via workspace:*');
  }
  if (Object.keys(manifest.peerDependencies ?? {}).length > 0) {
    throw new Error('@lostgradient/cinder-mcp must not declare peer dependencies');
  }
}
...
function publishedExport(entry: string | ConditionalExport): string | ConditionalExport {
  ...
  if (entry.import !== undefined) published.import = entry.import;
  if (entry.default !== undefined) published.default = entry.default;
  return published;
}

/**
 * Read the Cinder sibling package's currently-installed version. The staged
 * manifest pins `@lostgradient/cinder` to a `^<version>` range derived from THIS —
 * never a hand-maintained constant that could silently drift from what the
 * monorepo actually builds/publishes alongside this package.
 */
async function readCinderVersion(): Promise<string> {
  const cinderPackageJsonPath = join(PACKAGE_ROOT, '..', 'components', 'package.json');
  const parsed = parsePackageManifest(await Bun.file(cinderPackageJsonPath).text());
...
export async function buildPublishedManifest(source: PackageManifest): Promise<PackageManifest> {
  ...
  const published: PackageManifest = {
    ...
    dependencies: {
      ...source.dependencies,
      '@lostgradient/cinder': `^${cinderVersion}`,
    },
    exports: Object.fromEntries(
      Object.entries(source.exports).map(([key, entry]) => [key, publishedExport(entry)]),
    ),
    files: ['dist', '!dist/**/*.test.*', '!dist/**/*.spec.*', '!dist/**/*.map', 'README.md'],
  };
...
function assertStagedExports(manifest: PackageManifest): void {
  ...
  for (const target of Object.values(manifest.bin ?? {})) {
    const path = join(STAGING_ROOT, target.replace(/^\.\//, ''));
    if (!existsSync(path)) throw new Error(`bin target ${target} is missing from staging`);
  }
}
...
export async function packForPublish(): Promise<PackForPublishResult> {
  ...
  if (!existsSync(join(PACKAGE_ROOT, 'dist', 'bin.js'))) {
