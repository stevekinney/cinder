...
describe('shouldSkipBuild', () => {
  ...
  it('never skips when CINDER_FORCE_BUILD=1 is set, even on a matching marker', async () => {
    ...
    const decision = await shouldSkipBuild(inputs);
    expect(decision.skip).toBe(false);
    // The force path still returns the pre-build hash so the forced rebuild
    // records an accurate marker instead of recomputing after the build (which
    // could capture sources mutated mid-build).
    if (!decision.skip) expect(decision.hash).toBe(hash);
  });
