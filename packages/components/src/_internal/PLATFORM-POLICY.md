...
| `overflow-anchor: none`                                               | 1 (scoped) | Opt-OUT only, on a scroll container that performs its own scroll anchoring. VirtualList's `dynamicSize` mode corrects the offset itself when a measured row differs from its estimate; leaving native anchoring on makes the browser apply the same delta as well and the viewport over-scrolls by the row's size difference. Never disable it on a container that is not doing manual correction, and never rely on it being _supported_ — where it is unsupported the browser simply does not anchor, which is the behavior the manual path already assumes. | —                       |
| `scrollbar-gutter: stable`                                            | 1 (scoped) | Reserves the gutter on a scroll container whose CONTENT WIDTH must not depend on scrollbar presence. VirtualList's `dynamicSize` mode invalidates measured row heights when the available width changes, so a scrollbar appearing and disappearing would otherwise drive an invalidation loop. Purely presentational where unsupported — the gutter simply is not reserved and the existing width-change invalidation still handles it correctly, just less efficiently.                                                                                       | —                       |
...
| `scrollend` event                                                     | 2          | Progressive enhancement for settle detection on native scroll containers (Carousel, ScrollArea drag). Feature-detected per element (`'onscrollend' in element`); the existing debounce-based settle timer is the required fallback and must produce an identical result. Never the sole path to a required state.                                                                                                                                                                                                                                              | —                       |
