...
- For JS-side decisions (e.g. choosing `scrollTo` behavior, gating an animation `setTimeout`), use the `useReducedMotion()` hook. It exposes a reactive `.current` boolean and is SSR-safe (returns `false` on the server). New JS code must use this hook rather than reading `window.matchMedia` directly; existing inline checks should migrate opportunistically.
