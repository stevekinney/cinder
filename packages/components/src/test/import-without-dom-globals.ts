 ...
 * (`document`/`window`) removed, then restores them. Returns the message of the
 * first error any specifier throws on evaluation, or `undefined` if every
 * specifier imported cleanly — a clean import under nulled globals is the
 * SSR-safety proof (a module-level `document`/`window` access would surface here
 * as a `… is not defined` throw instead of `undefined`).
 *
