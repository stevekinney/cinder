...
| `class`        | `(opaque)` | no       | —       | Additional class merged with `.cinder-search-field`. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                                               |
| `onclear`      | `(opaque)` | no       | —       | Fires when the clear button is clicked. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                         |
| `oninput`      | `(opaque)` | no       | —       | Fires on every keystroke with the current value. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                |
| `onsearch`     | `(opaque)` | no       | —       | Fires when the native `search` event triggers (Enter or programmatic dispatch). A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
