...
| `onchange`        | `(opaque)`            | no       | —       | Fires only for user-initiated committed value changes (click, arrow keys, Space/Enter). Never fires for hover preview or external prop synchronization. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
