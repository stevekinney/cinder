...
Set `detectSequenceGaps` only when `events` is the complete, unfiltered stream. When consecutive rendered `StreamEvent` entries both provide numeric `sequence` values and the later value is not the previous value plus one, the viewer inserts a sequence-gap marker before the later event. The marker shows the expected and received sequence numbers, for example `Sequence gap — expected 8, received 10`.
