...
| `connectionState` | `"connected"` \| `"connecting"` \| `"disconnected"` \| `"error"` | no       | —       | Current connection state. When provided, renders a ConnectionIndicator in the toolbar. Omit when the stream has no live transport.                                                                                                                                       |
...
| `events`          | `(opaque)`                                                       | yes      | —       | Events and additive boundary entries to render in chronological order, oldest first. Not expressible in JSON Schema; see the component types for the signature.                                                                                                          |
