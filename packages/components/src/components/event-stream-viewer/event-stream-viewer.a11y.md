...
- **Sequence gaps are advisory.** The component only inserts a gap marker when two consecutive rendered `StreamEvent` entries both provide numeric `sequence` values and the later sequence is not the previous sequence plus one. It does not validate server ordering, backfill missing events, or infer gaps across unnumbered events.
