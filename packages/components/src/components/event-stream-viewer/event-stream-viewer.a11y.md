...
- **Sequence gaps are advisory.** The component only inserts a gap marker when `detectSequenceGaps` is true, the current `filterQuery` is blank, and two consecutive rendered `StreamEvent` entries both provide numeric `sequence` values where the later sequence is not the previous sequence plus one. It does not validate server ordering, backfill missing events, or infer gaps across filtered, retained, or unnumbered events.
