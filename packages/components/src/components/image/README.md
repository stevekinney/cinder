...
| `class`       | `(opaque)`                        | no       | —       | Additional class names merged with `.cinder-image`. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                        |
| `fallback`    | `(opaque)`                        | no       | —       | Rendered in place of the `<img>` when it fails to load. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
| `onerror`     | `(opaque)`                        | no       | —       | Forwarded after internal state updates. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                 |
| `onload`      | `(opaque)`                        | no       | —       | Forwarded after internal state updates. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                 |
