...
<JsonEditor id="payload" label="Payload" {value} onchange={(next) => (value = next)} />
```
...
| `onchange`          | `(opaque)` | no       | —       | Called with the proposed JSON source whenever the user edits the textarea. Not expressible in JSON Schema; see the component types for the signature. |
