...
| `original`      | `(opaque)`                                 | no       | —       | Optional explicit baseline; defaults to the initial `schema`. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.             |
| `schema`        | `(opaque)`                                 | yes      | —       | The schema being edited. May be a string (JSON text) or pre-parsed value. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature. |
