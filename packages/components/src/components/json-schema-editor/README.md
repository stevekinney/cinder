...
| `onSchemaChange`       | `(opaque)`                                 | no       | —       | Observe a committed schema change: locally immediately, or after controlled acceptance. Not expressible in JSON Schema; see the component types for the signature.      |
...
| `onValueChangeRequest` | `(opaque)`                                 | no       | —       | With `schema`, activates controlled state and requests each committed parent update. Not expressible in JSON Schema; see the component types for the signature.         |
...
| `schema`               | `(opaque)`                                 | no       | —       | Current schema. With `onValueChangeRequest`, parent-owned; otherwise, a local initial value. Not expressible in JSON Schema; see the component types for the signature. |
