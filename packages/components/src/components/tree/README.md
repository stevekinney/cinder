...
| `children`          | `(opaque)`                             | yes      | —       | Tree items (snippet). A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                |
| `selectionControls` | `(opaque)`                             | no       | —       | Optional selection controls rendered before the role="tree" element. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
