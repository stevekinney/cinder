...
| `onchange`          | `(opaque)`                                                                                                                                                               | no       | —       | Called whenever the user makes any edit. Required for editable runtime usage; readonly schema-driven usage may omit it because no edit controls are rendered. Receives the next rule array (pure, not mutated) and a change descriptor. Consumer owns persistence, validation, and execution. Not expressible in JSON Schema; see the component types for the signature. |
