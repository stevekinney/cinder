...
| `operatorOptions`   | ({ label: `string`; options?: { label: `string`; value: `string` }[]; type?: `"string"` \| `"number"` \| `"boolean"` \| `"enum"`; value: `string` })[]                   | no       | —       | Options for the condition operator selector. Consumer-provided list of operators, e.g. "matches", "is", "is-not", "contains". Required in full mode; optional (and ignored) when `mode` is `'conditions'`, since cinder supplies a fixed operator set in that mode — so a conditions-only configuration validates without it.                                                                             |
