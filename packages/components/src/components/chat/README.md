...
`oncomposerkeydown` runs before Chat's internal Enter-to-send handling. If the
callback calls `event.preventDefault()`, Chat skips its internal key handling
for that event. When the callback does not prevent the default, Enter-to-send,
Shift+Enter newline behavior, and IME composition handling remain unchanged;
during IME composition, Enter confirms the candidate instead of sending.
...
| `oncomposerkeydown`                | `(opaque)`                                     | no       | —       | Called before Chat's internal composer key handling when a keydown originates from the composer textarea. Call `event.preventDefault()` to let an overlay consume Arrow keys, Enter, or Escape before Enter-to-send. The internal handler still ignores IME composition events when the callback does not prevent the default. Not expressible in JSON Schema; see the component types for the signature.                              |
