...
Chat's root (`.chat-container`) is `height: 100%`, so it always fills its
parent. The flip side: if no ancestor on the chain resolves to a definite
height, `height: 100%` resolves against `auto` and Chat collapses to the
intrinsic height of its empty state plus composer — a small card, not the
full surface you expect. There is no browser error; the layout just silently
shrinks. Give Chat a sized ancestor with one of the two patterns below.
...
The same shape works inside a grid cell: give the cell a resolved track size
(e.g. `grid-template-rows: auto 1fr`) and place Chat in the `1fr` row.
...
| `messagePart`             | `(opaque)`                     | no       | —       | Per-message-part override. Replaces the rendering of an individual body part (text, markdown, tool call, tool result) while delegating the rest to the built-ins via the `renderDefault` snippet it receives. Not expressible in JSON Schema; see the component types for the signature.           |
...
| `onadaptererror`          | `(opaque)`                     | no       | —       | Called when an awaited adapter command rejects. Not expressible in JSON Schema; see the component types for the signature.                                                                                                                                                                         |
