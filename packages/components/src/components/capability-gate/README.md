...
| `actions`          | `(opaque)`                                                                                                           | no       | —          | Action row content — compose your own Buttons/links (primary, fallback, dismiss, anything else). The snippet receives a `dismiss` function that runs the gate's own unmount-and-`onDismiss` path, so a consumer dismiss button gets the component's focus handling for free: `svelte {#snippet actions({ dismiss })}   <Button label="Allow access" onclick={requestAccess} />   <Button variant="secondary" label="Not now" onclick={dismiss} /> {/snippet} ` Not expressible in JSON Schema; see the component types for the signature. |
