...
- Repeated presses advance from where the previous one was heading. Under `smoothScroll` the scroll position lags the animation, so without this a held arrow key resolves the same destination over and over and advances one row for many presses.
