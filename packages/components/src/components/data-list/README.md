...
| `children` | `(opaque)`                       | yes      | —       | Row renderer. MUST render an `<li>` (the list root is a `<ul role="list">`). StackedListItem is the recommended row — it renders an `<li>` with leading/title/description/meta/trailing slots. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                        |
| `empty`    | `(opaque)`                       | no       | —       | Rendered when `items` is empty. The component automatically wraps the snippet output in `<li class="cinder-data-list-empty">`. **Do NOT wrap in an `<li>` yourself** — the component provides the `<li>` wrapper automatically. Pass only inner content (e.g. a `<p>`, a `<div>`, or plain text). Contrast with `children`, which must render an `<li>`. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                              |
| `items`    | `(opaque)`                       | yes      | —       | The records to render. Each is passed to `children`. A generically typed prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `key`      | `(opaque)`                       | no       | —       | Key extractor for efficient DOM updates. Svelte uses this to identify each row when the list is reordered, filtered, or updated — without it, rows are matched by index and the wrong row instances may receive updated props. Strongly recommended for any list that can change after initial render: `svelte <DataList {items} key={(m) => m.id}> ` Omit only for truly static, never-reordered lists (e.g. a fixed reference list). When omitted, Svelte falls back to index-based reconciliation. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
