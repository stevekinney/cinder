...
| `status`    | `"online"` \| `"offline"` \| `"warning"` \| `"danger"` \| `"pending"` \| `"neutral"` \| `"success"` \| `"accent"` | yes      | —       | Required semantic status. Drives color via `data-cinder-status`. `success` maps to `--cinder-success`; `accent` maps to `--cinder-accent`. |
