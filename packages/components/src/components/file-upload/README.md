...
| `fileList`          | `(opaque)` | no       | —                                 | Replaces the default file-list renderer. Receives the resolved rows and a removal callback when the queue is mutable. Not expressible in JSON Schema; see the component types for the signature. |
