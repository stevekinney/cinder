...
| `children` | `(opaque)`                                                 | yes      | —       | Item content — label text, an icon, or richer markup. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
