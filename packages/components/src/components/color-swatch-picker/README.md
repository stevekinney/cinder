...
| `colors`       | `(opaque)`                                     | yes      | —       | Palette to render. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                         |
| `indicator`    | `(opaque)`                                     | no       | —       | Snippet that replaces the default check-icon indicator on the selected swatch. Receives the active swatch and the computed contrast color for the icon. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
| `onchange`     | `(opaque)`                                     | no       | —       | Fired when the selected swatch changes. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                 |
