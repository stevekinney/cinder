...
| `children`       | `(opaque)`              | no       | —       | Main event body — the description, links, and secondary metadata. Passed as the default children snippet: `svelte <FeedEvent datetime="…" timestamp="2m ago">   <strong>{user.name}</strong> pushed 3 commits </FeedEvent> ` A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                                                  |
| `icon`           | `(opaque)`              | no       | —       | Required for the icon variant. Type-enforced by the discriminated union. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `timestampLabel` | `(opaque)`              | no       | —       | Rich visible time label, for the rare case where the label needs markup (e.g. an `<abbr>` or nested element). Takes precedence over `timestamp` when both are supplied. Most consumers should use the `timestamp` string instead. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                                             |
