...
| `onPause`       | `(opaque)`                  | no       | —           | Called when the pause action is triggered. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.  |
| `onPlay`        | `(opaque)`                  | no       | —           | Called when the play action is triggered. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.   |
| `onReplay`      | `(opaque)`                  | no       | —           | Called when the replay action is triggered. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
