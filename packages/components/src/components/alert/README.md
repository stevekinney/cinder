...
| `variant`     | `"info"` \| `"success"` \| `"warning"` \| `"danger"` \| `"error"` | no       | `"info"` | Visual severity variant. `status.danger.solid` is the canonical failure-severity spelling, consistent with banner and callout. `error` remains accepted as a deprecated alias. |
