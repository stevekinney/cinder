...
| `children`       | `(opaque)` | yes      | —       | Items — typically `GridListItem` instances. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                          |
| `class`          | `(opaque)` | no       | —       | Extra class names merged with `cinder-grid-list`. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                       |
