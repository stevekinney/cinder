...
Render an accessible form from a JSON Schema object or Standard Schema validator and submit one validated value.
...
| `schema`      | `(opaque)` | yes      | —       | JSON Schema object or Standard Schema object used to render and validate the form. Not expressible in JSON Schema; see the component types for the signature. |
| `value`       | `(opaque)` | no       | —       | Initial form value. Missing fields are seeded from the schema where possible. Not expressible in JSON Schema; see the component types for the signature.      |
