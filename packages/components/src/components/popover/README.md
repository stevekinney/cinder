...
| `children`        | `(opaque)`                                                                                                           | yes      | —       | Panel content. Required. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                            |
| `trigger`         | `(opaque)`                                                                                                           | no       | —       | Optional trigger snippet rendered inside a wrapper. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
| `triggerRef`      | `(opaque)`                                                                                                           | no       | —       | Explicit anchor element. Wins over the snippet-resolved focusable. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.     |
