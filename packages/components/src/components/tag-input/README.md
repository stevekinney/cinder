...
| `autocomplete`     | `(opaque)`                                                                                                    | no       | —       | Autocomplete hint forwarded to the visible text input. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                          |
| `delimiter`        | `(opaque)`                                                                                                    | no       | —       | Key that commits the current input into a tag. Enter always commits separately. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature. |
