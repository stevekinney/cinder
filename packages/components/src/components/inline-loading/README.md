...
| `status`          | `"inactive"` \| `"active"` \| `"finished"` \| `"error"` | no       | `"inactive"` | Lifecycle state for the inline async action indicator. Bindable: the component resets this to `"inactive"` after the `successDelay` timer fires so a subsequent `status = "finished"` from the parent is always a real value transition that re-shows the success indicator. |
