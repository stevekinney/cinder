...
| `children` | `(opaque)` | yes      | —       | Feed events (typically `<FeedEvent>` children). A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                |
