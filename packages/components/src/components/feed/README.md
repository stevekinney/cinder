...
| `following`       | `boolean`                                                        | no       | `true`           | Log arm only — requires `kind: 'log'`; rejected by the list arm. When true, appended content automatically scrolls the viewport to the bottom. Scrolling away from the bottom pauses following; scrolling back to the bottom (or the built-in control) resumes it. Bindable so the parent can read the paused state the component sets internally.                                                        |
