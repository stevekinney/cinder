...
| `children` | `(opaque)`                                  | no       | —           | The icon (or icon + label for extended variant). Always provide `aria-label` when the FAB renders an icon without visible text — i.e. the `filled` variant. Not expressible in JSON Schema; see the component types for the signature. |
