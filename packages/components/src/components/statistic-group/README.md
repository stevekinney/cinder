...
| `variant`  | `"default"` \| `"cards"` \| `"shared-borders"` | no       | `"default"` | Visual variant; surfaced as `data-cinder-variant` for CSS styling. - `'default'` — a bordered resting surface with dividers between stats. - `'cards'` — each stat gets a card-style border and shadow. - `'shared-borders'` — single outer border with 1px gap dividers between stats. |
