...
| `children` | `(opaque)`                          | no       | —       | Cell content. Optional so that empty `<td>` cells (used in spanning table layouts) are a valid, non-throwing state. When omitted the cell renders empty, which is valid HTML for a `<td>`. **Note for TypeScript consumers:** the Snippet type is never called externally by consuming code — Svelte's compiler handles invocation internally. Making this optional is therefore safe as an API change: no external caller calls `props.children()` on a Svelte component's props. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
