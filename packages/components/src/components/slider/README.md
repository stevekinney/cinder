...
| `mode`          | `"single"` \| `"range"`         | no       | —       | Slider mode. `"single"` renders one thumb and emits a scalar value; `"range"` renders two thumbs and emits a `[low, high]` tuple. Default `"single"`. |
...
| `unit`          | `string`                        | no       | —       | Optional unit displayed with the current value, such as `%` or `ms`.                                                                                  |
