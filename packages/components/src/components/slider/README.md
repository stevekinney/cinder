...
| `valueText`    | `(opaque)`                      | no       | —       | Formats the numeric value for `aria-valuetext`. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
