...
| `children` | `(opaque)`                                           | yes      | —       | Callout body content. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `icon`     | `(opaque)`                                           | no       | —       | Optional decorative icon rendered inside an `aria-hidden` wrapper. The icon is a second visual channel alongside variant color — it helps satisfy WCAG 1.4.1 (Use of Color) by ensuring the variant is conveyed by more than color alone. The accessible meaning still lives in `title` and/or `children`; the icon must not be the only carrier of information. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                         |
