...
Per the WAI-ARIA pattern, only one tab sits in the tab order (`tabindex=0`); the rest are reachable via arrow keys (`tabindex=-1`). The selected tab takes that tab stop _when it is enabled_. If the selected tab is disabled — at mount, or because `disabled` toggled true after mount — the tab stop falls back to the first enabled tab in source order. On the client, this guarantees a reachable entry point at all times. In server-rendered HTML specifically, this fallback cannot be resolved before the client attaches (Svelte's snippet-based children give the server no way to know about a not-yet-rendered sibling), so a misconfigured `Tabs` — one whose selected value is invalid, or whose initially-selected tab is disabled — renders with **no** `tabindex="0"` tab in the raw server HTML; the correct tab stop appears the instant the client mounts. A `Tabs` whose initially-selected tab is valid and enabled (the common case, in any tab position) always has exactly one correct tab stop, including in server-rendered HTML. Selection (`aria-selected`) is preserved on the disabled tab; only the tab stop moves.
