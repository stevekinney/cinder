...
<script lang="ts">
  ...
  import type { FloatingActionProps } from './floating-action.types.ts';
  ...
  const mergedClassName = $derived(classNames('cinder-floating-action', customClassName));
  ...
  $effect(() => {
    ...
    // Filled floating actions are icon-only by design — they require an accessible name.
    if (shape === 'filled' && !hasAriaLabel && !hasAriaLabelledBy) {
      devWarn(
        '[cinder/FloatingAction] filled shape requires aria-label or aria-labelledby — the icon alone does not provide an accessible name.',
      );
    }

    // All floating actions need some accessible name.
    if (!hasAriaLabel && !hasAriaLabelledBy && !children) {
      devWarn(
        '[cinder/FloatingAction] rendered without an accessible name — pass aria-label, aria-labelledby, or children.',
      );
    }
  });
</script>
