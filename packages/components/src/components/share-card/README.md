...
| `actions`         | `(opaque)` | no       | —       | Explicit actions to show. When omitted, default copy + native-share actions render. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature. |
| `preview`         | `(opaque)` | no       | —       | Preview content slot rendered above the actions. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                 |
