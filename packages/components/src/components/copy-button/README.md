...
| `children`        | `(opaque)` | no       | —       | Default content (idle state). A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                                             |
| `confirmation`    | `(opaque)` | no       | —       | Content rendered while in the "copied" state. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                                             |
