...
| `empty`       | `(opaque)` | no       | —       | Rendered when zero items are registered after filtering. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                 |
| `footer`      | `(opaque)` | no       | —       | Optional footer, e.g. for keybinding hints. Not part of the listbox. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                     |
| `items`       | `(opaque)` | yes      | —       | Receives the current query so consumers can filter. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                      |
| `onclose`     | `(opaque)` | no       | —       | Fired after any close path routed through the palette close lifecycle. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                   |
| `triggerRef`  | `(opaque)` | no       | —       | Element to restore focus to on close. Falls back to `captureFocus()`. A prop whose shape is not captured by the JSON schema; see the component types for the exact signature.                                                                       |
