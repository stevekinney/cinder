...
- Always provide a `label` unless the input's purpose is unambiguous from surrounding context (e.g., a search field immediately following a "Search" heading). For compact layouts, keep `label` and set `labelVisible` instead of omitting the label.
