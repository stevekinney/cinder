...
const schema = {
  ...
  properties: {
    ...
    type: {
      enum: ['number', 'date', 'email', 'password', 'search', 'tel', 'text', 'time', 'url'],
      description:
    ...
    labelVisible: {
      type: 'boolean',
      description:
        'Whether the `label` is visibly rendered. Set `false` to visually hide\nit while keeping it programmatically associated.',
      default: true,
    },
    description: {
      type: 'string',
