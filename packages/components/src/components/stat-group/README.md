...
| `children` | `(opaque)`                                     | yes      | —             | Stat children, typically one or more `<Stat>` components. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature.                                                                                   |
