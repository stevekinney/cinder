...
- **Enter key.** Commits synchronously. After a successful commit, when `enterBehavior='commit-then-submit'` (default), the handler `preventDefault()`s the native submit and re-issues `form.requestSubmit()` with the canonical value already written to the hidden mirror. On an empty or invalid value, the handler does not suppress the keypress — but the field calls `setCustomValidity()` with the parse error, so the surrounding form's native validation blocks submission of stale data.
