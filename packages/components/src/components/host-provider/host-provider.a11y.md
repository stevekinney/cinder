...
The provider renders a `display: contents` node and introduces no ARIA role, focus target, keyboard interaction, announcement, or motion. Desktop drag regions automatically punch common interactive descendants back out, and custom interactive islands must use `cinder-no-drag`. The design and accessibility review accepted the wrapper-free layout behavior and required the explicit no-drag discipline so native window movement never consumes control input.
