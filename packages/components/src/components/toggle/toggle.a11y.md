...
The component renders its own label as a `<span id="${id}-label">` next to the switch, and the `<button>` references it via `aria-labelledby`. The label is always present (visually hidden via `labelVisible` when needed), so it is the single source of the accessible name — `aria-label` is not used.
