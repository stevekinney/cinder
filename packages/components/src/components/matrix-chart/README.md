...
| `loadingContent`      | `(opaque)`                                          | no       | —       | Snippet rendered while the chart is loading. A function or snippet prop. Its shape is not captured by the JSON schema; see the component types for the exact signature. |
