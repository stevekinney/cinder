...
async function main(): Promise<void> {
  ...
  assertNoForbiddenExportKeys(packageJson.exports, FORBIDDEN_EXPORT_KEY_PATTERN);
  ...
  assertNoForbiddenExportKeys(next, FORBIDDEN_EXPORT_KEY_PATTERN);
  ...
  process.stdout.write(
    `exports:generate — wrote ${Object.keys(computed).length} computed subpaths ` +
      `(${components.length} components) and ${packageJson.files.length} files entries\n`,
  );
}
