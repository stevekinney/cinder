...
/**
 * Human-facing component authoring checklist.
 *
 * Keep this list as the machine-readable source of truth for the scaffold
 * README and the authoring guidance. The repository and skill mirrors link
 * back to this list so a new convention has one place to update.
 */
export const COMPONENT_AUTHORING_CHECKLIST = [
  {
    id: 'existing-primitive',
    text: 'Search the component inventory and compose an existing primitive before creating a new one.',
    references: ['#919', '#929'],
  },
  {
  ...
  },
  {
    ...
    text: 'Apply the `cinder-_floating-surface` class to floating panels (defined in `_floating-surface.css`), and compose form controls from `Input` and `FormField`.',
    references: ['#921', '#923'],
  },
  {
    id: 'shared-chrome',
  ...
  },
  {
    ...
    text: 'Add tests for resulting state and resting appearance, not only transitions.',
    references: ['#931'],
  },
  {
  ...
  {
    id: 'novel-interaction-accessibility-review',
    text: 'For a novel interaction model, get an accessibility review covering focus management, the keyboard matrix, and assistive-technology announcements; record the outcome in `*.a11y.md`.',
    references: ['#968'],
  },
] as const;

/** Render the checklist for documentation generated by components:create. */
export function renderComponentAuthoringChecklist(): string {
  return COMPONENT_AUTHORING_CHECKLIST.map(
    ({ text, references }) => `- [ ] ${text} (${references.join(', ')})`,
  ).join('\n');
}
...
function hasBrowserGuardInSource(source: string): boolean {
  ...
  if (!isObjectRecord(ast)) return false;
...
function scriptImportsBrowserFromEsmEnv(scriptNode: unknown): boolean {
  if (!isObjectRecord(scriptNode)) return false;
  ...
  for (const statement of body) {
    ...
    for (const specifier of specifiers) {
      if (!isObjectRecord(specifier)) continue;
