...
function isOutsideCoverageScope(file: string, scope: CoverageScope): boolean {
  ...
  // The CLI is tested as a subprocess so entrypoint behavior and process I/O
  // match the published binary. Bun does not merge subprocess LCOV into the
  // parent report, so keep it out of the in-process runtime ratchet.
  if (normalizedFile.startsWith('src/cli/')) return true;
