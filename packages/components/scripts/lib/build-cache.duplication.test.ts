// `buildCache` is duplicated byte-for-byte into five packages because each
...
// silent DRIFT between the copies is the danger: a fix applied to one and not
...
function copyPath(packageName: string): string {
  ...
  return join(import.meta.dir, '..', '..', '..', packageName, 'scripts', 'lib', 'build-cache.ts');
}

describe('build-cache copies stay byte-for-byte identical', () => {
  it('lists every package that should carry a copy (catches a forgotten new one)', async () => {
    // A new package that builds to dist via this helper must be added here.
  ...
  it('every copy is byte-for-byte identical to the canonical components copy', async () => {
    ...
    for (const packageName of PACKAGES_WITH_COPY) {
      ...
      expect(await file.exists(), `${packageName} is missing its build-cache.ts copy`).toBe(true);
      ...
      expect(
        identical,
        `${packageName}/scripts/lib/build-cache.ts has DRIFTED from the canonical copy`,
      ).toBe(true);
    }
  });
});
