 ...
 * `for=` / `aria-labelledby=` / `aria-controls=`), and template interpolation
 * `` `#${mainId}` `` → `"#main"`.
 *
 * ## Fail-closed
 *
...
const DERIVED_ID_LINE =
  /^(?:[^\S\n]*)(?:let|const)\s+(?<var>[A-Za-z_$][\w$]*)\s*=\s*\$derived\([\t ]*(?:\n[\t ]*)?`\$\{mountIdPrefix\s*\?\?\s*uid\}-(?<suffix>[^`]*)`[\t ]*,?[\t ]*(?:\n[\t ]*)?\)\s*;?[^\n]*(?:\n|$)/m;
...
export function stripExampleHarness(source: string, scenarioKey: string): string {
  ...
  for (const [variable, literal] of literals) {
    ...
    // aria-labelledby=/aria-controls=, e.g. a `<label for={fieldId}>` paired with the field).
    stripped = stripped.replace(
      new RegExp(
        `(\\b(?:id|name|target|for|aria-labelledby|aria-controls)=)\\{\\s*${escaped}\\s*\\}`,
        'g',
      ),
