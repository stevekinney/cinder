...
describe('stripCommentsRespectingStrings', () => {
  test('replaces // line comments with spaces to end of line', () => {
    const result = strip('const x = 1; // @cinder/editor');
    ...
    expect(result.length).toBe('const x = 1; // @cinder/editor'.length);
  });
  ...
  test('marks the rest of the line as comment when /* is unclosed', () => {
    ...
    const result = strip('const x = 1; /* @cinder/editor', state);
    expect(result).not.toContain('@cinder/');
...
describe('lineHasUpstreamSpecifierResidue', () => {
  ...
  test('flags backtick-quoted no-interpolation specifier', () => {
    expect(scan('await import(`@cinder/editor`);')).toBe(true);
  });
  ...
  test('does not flag a JSDoc continuation line with backticks (state-tracked)', () => {
    ...
    expect(scan(' * Mention `@cinder/editor` in prose', state)).toBe(false);
    expect(state.inBlockComment).toBe(true);
  });

  test('does not flag a `//` line comment with `@cinder/`', () => {
    expect(scan('// see `@cinder/editor` for details')).toBe(false);
  });
  ...
  test('tracks multi-line block comments across calls and resumes after `*/`', () => {
    ...
    expect(state.inBlockComment).toBe(true);
    expect(scan(' * @cinder/editor reference', state)).toBe(false);
  ...
  test('flags `/* x */ import from "@cinder/y"` — the single-line block-comment bypass case', () => {
    expect(scan(`/* ignore */ import x from '@cinder/editor';`)).toBe(true);
  });

  test('flags `import x from "@cinder/y" /* trailing */`', () => {
    expect(scan(`import x from '@cinder/editor' /* trailing */;`)).toBe(true);
  });

  test('does not flag a single-line block comment containing `@cinder/`', () => {
    expect(scan(`/* see @cinder/editor */`)).toBe(false);
  });

  test('does not flag a line ending with `//` after stripping', () => {
    expect(scan(`const message = 'plain'; // @cinder/editor is unused here`)).toBe(false);
  });
  ...
  test('tracks multi-line block comments and resumes scanning after `*/`', () => {
    ...
    expect(state.inBlockComment).toBe(true);
    expect(scan(` closes */ import x from '@cinder/editor';`, state)).toBe(true);
    expect(state.inBlockComment).toBe(false);
  });
  ...
  test('does not flag `@cinder/...` references that only appear inside string literals', () => {
    ...
    expect(scan(`const description = 'see @cinder/editor for details';`)).toBe(false);
  });

  test('does not flag a published sibling package specifier (legitimate external dependency)', () => {
    // `@lostgradient/markdown` is a real, external runtime dependency of
    // cinder now (see `src/utilities/change-tracker.svelte.ts`) — a bare
    // specifier in built output is correct, not residue. Only the
    // workspace-private `@cinder/*` scope is ever forbidden in built output.
    expect(scan(`import x from '@lostgradient/markdown';`)).toBe(false);
    expect(scan(`import { render } from '@lostgradient/markdown/rendering';`)).toBe(false);
  });
});

describe('containsUpstreamSpecifier', () => {
  test('matches any @cinder/* specifier', () => {
    expect(containsUpstreamSpecifier(`import x from '@cinder/editor';`)).toBe(true);
  });

  test('does not match a published sibling package specifier', () => {
    expect(containsUpstreamSpecifier(`import x from '@lostgradient/markdown';`)).toBe(false);
  });
