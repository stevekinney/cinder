...
/**
 * `@lostgradient/markdown` is headless (no Svelte), so unlike `@lostgradient/chat` it declares
 * real `dependencies` (remark/rehype/unified/shiki/etc) rather than routing
 * everything through peers. The only thing the source manifest must NOT
 * contain is a leftover `workspace:*` protocol reference.
 */
export function assertSourceManifest(manifest: PackageManifest): void {
  const serialized = JSON.stringify(manifest.dependencies ?? {});
  if (serialized.includes('workspace:')) {
    throw new Error('@lostgradient/markdown dependencies must not use the workspace: protocol');
  }
  if (Object.keys(manifest.peerDependencies ?? {}).length > 0) {
    throw new Error('@lostgradient/markdown is headless and must not declare peer dependencies');
  }
}
