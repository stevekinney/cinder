 ...
 * guard the ways that setup silently rots:
 *
 *   (a) The function entry stops exporting the `{ fetch }` shape Vercel calls,
 *       or stops delegating to the real handler.
 *   (b) Someone adds a new route to `playground-server.ts` (or removes one) and
 *       the `vercel.json` rewrites drift out of sync, so the new route 404s in
 *       production while working locally. Each route family the server answers
 *       must have a rewrite that reaches the function.
 *   (c) The server module is renamed back to a Vercel backend-entrypoint magic
 *       name (`server`/`index`/`app`/`main` in root or `src/`). That would make
 *       Vercel auto-detect it as a root entrypoint and switch into backend mode,
 *       which ignores the `api/` function — so `includeFiles` never bundles the
 *       components tree and every `/page-bundle/*` build 500s in production. This
 *       is the exact failure this config was renamed to avoid; the test pins the
 *       module out of that name set.
 *
...
/**
 * Narrow the parsed config's `rewrites` to an array of source strings. Throws
 * if the shape is wrong so a malformed rewrite table fails loudly.
 */
...
describe('vercel.json', () => {
  ...
  it('registers exactly one function — the api/index.ts entry — with the components source bundled in', async () => {
    ...
    // functions keys MUST be under api/ (Vercel rejects e.g. "src/server.ts"
    ...
    // time, so those trees must be bundled into the deployed function. This is
    // the ONLY mechanism that ships them — and it requires the api/ function key.
  ...
  it('rewrites every playground route family to the single function', async () => {
    ...
    // answers. If a new family is added to playground-server.ts, add it here AND
    // to vercel.json — this list is the contract between the two.
  ...
  it('keeps the server module out of Vercel backend-entrypoint auto-detection', async () => {
    // Vercel's Bun backend mode auto-detects a root entrypoint named
    // server/index/app/main (in root or src/). If the server module had such a
    // name, Vercel would run IT as the root function and ignore the api/ entry —
    // dropping includeFiles and 500-ing every bundle build. Assert the file is
    // named so that never happens.
    const serverModule = Bun.file(join(PLAYGROUND_ROOT, 'src', 'playground-server.ts'));
    expect(await serverModule.exists()).toBe(true);
    const reserved = ['server', 'index', 'app', 'main'];
    expect(reserved).not.toContain('playground-server');
    // And the legacy magic name must not exist alongside it.
