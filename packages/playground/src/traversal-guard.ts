 ...
 * lexically escape it via a leading `..` segment, a leading `/`, or a
 * `join()`-then-`relative()` round-trip that lands outside `baseDir`.
 * Returns the absolute path, or `null` when the request is unsafe.
 *
