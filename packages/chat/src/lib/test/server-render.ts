  ...
  return {
    ...
    setup(builder) {
      ...
      builder.onResolve({ filter: /\.svelte\.(js|ts)$/ }, ({ path, importer, resolveDir }) => {
        ...
        const typeScriptSourcePath = resolvedPath.replace(/\.svelte\.js$/, '.svelte.ts');
        return {
          path: existsSync(typeScriptSourcePath) ? typeScriptSourcePath : resolvedPath,
          namespace,
        };
      });
...
export async function renderToServerHtml<Props extends Record<string, unknown>>(
  ...
  // tree. A `.mjs` extension keeps Bun's `.svelte.ts` preload plugin from trying
