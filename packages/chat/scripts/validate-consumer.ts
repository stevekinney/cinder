...
async function buildConsumerEntries(fixture: ValidationFixture): Promise<void> {
  ...
  await Bun.write(
    clientEntryPath,
    `import Chat, { ArtifactPanel, appendToolCall, appendToolCalls, appendToolResult, appendToolResultAsync, appendToolResults, appendToolResultsAsync, pairToolCallsWithResults, createChatSessionController, decodeChatStreamEvent, encodeChatStreamEvent } from '@lostgradient/chat';\n` +
      ...
      `if (typeof createChatSessionController !== 'function') throw new Error('missing session controller export');\n`,
  );
  await Bun.write(
    serverEntryPath,
