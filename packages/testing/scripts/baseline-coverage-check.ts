...
export function expectedBaselineFileNames(
  ...
  const add = (slug: string, theme: Theme, viewport: ViewportName, fixture: string): void => {
    const fileName = basename(snapshotPath({ slug, theme, viewport, fixture }));
    const bucket = expected.get(slug);
    if (bucket === undefined) {
      expected.set(slug, new Set([fileName]));
      return;
    }
    bucket.add(fileName);
  };
  ...
  for (const entry of entries) {
    ...
    for (const theme of THEMES) {
      for (const viewport of VIEWPORTS) {
        for (const fixture of fixtures) {
          add(entry.slug, theme, viewport.name, fixture.name);
        }
      }
    }
  }
...
export function evaluateBaselineCoverage(
  ...
  return {
    ...
    missing: findMissingBaselines(adopted),
