...
**Domain-suite consumer-facing components** (current `cinder/<name>` allowlist):

These are the public components consumers import directly.
...
**Domain-suite supporting workspace packages** (not consumer-facing `cinder/<name>` components):
...
- `@cinder/diff` — standalone diff algorithm; `diff-match-patch` only dep. Consumers do not import this directly; `cinder/diff-viewer` imports it.
- `@cinder/markdown` — markdown pipeline, rendering, and utilities; 18 npm deps (unified, remark-_, rehype-_, shiki, etc.). Not a `cinder/<name>` component — it is a workspace library.
...
| Markdown Renderer (via `cinder/markdown-editor` + `cinder/diff-viewer`) | —                                | ✓      | —                            | ✗                        | **domain-suite, Phase 6** (no standalone renderer; `@cinder/markdown` is a workspace package, not a component) |
