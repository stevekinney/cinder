...
| `bun run test:unit`       | Runs the fast unit tests (`check-upstream-issues.test.ts` and `chat-server.test.ts`). This is what `bun run test` aliases, so `turbo run test` at the workspace root stays fast — it does **not** exercise application behavior. |
