...
| `bun run test:unit`           | Runs the fast unit tests (`check-upstream-issues.test.ts`, `chat-server.test.ts`, and `chat-agent.test.ts`). This is what `bun run test` aliases, so `turbo run test` at the workspace root stays fast — it does **not** exercise application behavior. |
| `bun run test`                | Alias for `test:unit`, not the browser suite — see above.                                                                                                                                                                                               |
| `bun run test:live-operative` | One real, billed request against the live Anthropic API through the same `/api/chat` pump, requiring a real `ANTHROPIC_API_KEY` in the environment. **Not** run by `test`, `test:unit`, or CI — invoke it by hand only.                                 |
