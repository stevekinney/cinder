...
| `bun run test:unit`       | Runs the fast unit tests (`check-upstream-issues.test.ts`, `chat-server.test.ts`) plus `playwright-collision-guard.test.ts`, which holds each `webServer` port with a fake server and spawns Playwright to prove it refuses instead of adopting it (about a second per port; needs 4599, 4173, and 5175 free). This is what `bun run test` aliases, so `turbo run test` at the workspace root stays fast — it does **not** exercise application behavior. |
