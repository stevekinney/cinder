...
| `bun run test:unit`       | Runs the fast unit tests (`check-upstream-issues.test.ts`, `chat-server.test.ts`) plus `playwright-collision-guard.test.ts`, which pins `reuseExistingServer: false` on every `webServer` entry and, for the first entry, holds its port (4599) with a fake server and spawns Playwright to prove it refuses instead of adopting it (about a second; needs 4599 free — Playwright refuses before it reaches the other two ports). This is what `bun run test` aliases, so `turbo run test` at the workspace root stays fast — it does **not** exercise application behavior. |
