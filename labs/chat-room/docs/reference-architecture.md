...
- **Run errors**: provider, generation, output-validation, guardrail, budget, and runtime failures become a terminal `run.error` or `run.tripwire` frame. Operative's error values are typed as `unknown`, so the **host owns** the safe `kind`, `code`, and `message` envelope. It never serializes an unknown error object or a credential-bearing provider response directly.
