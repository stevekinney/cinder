...
The adoption target is `@lostgradient/operative@0.7.0`, installed from the npm registry. The first adoption pull request must declare the dependency as the exact string `"0.7.0"` in `package.json`, with no caret, tilde, tag, or other range, and commit the corresponding registry resolution in `bun.lock`. No downstream implementation may substitute a workspace link, source checkout, local tarball, patch, override, or another version.

As of September 1, 2026, npm publishes `0.1.0` through `0.7.0`. [AB-24](https://linear.app/lost-gradient/issue/AB-24/publish-and-externally-verify-lostgradientoperative020), which once gated this, is Done. Note that AB-24 recorded its end-to-end external verification against `0.6.0`; `0.7.0` shipped three hours later and carries no verification run of its own, which is why the registry check below is a required step of the adoption pull request rather than a formality.

This document originally split its claims into a published `0.1.0` baseline and an unpublished `0.2.0` adoption contract, because at the time of writing only `0.1.0` existed. That split is now historical: the typed Agent API is published and the two levels have collapsed into one. The distinction is retained below only to show which claims were verified against a real artifact and which came from the specification.

- **Carried over from the `0.1.0` baseline, not re-verified symbol by symbol**: `@lostgradient/operative@0.7.0` is expected to publicly export `createAgent`, `AgentRun`, `RunEvent`, `RunResult`, `stopWhen`, `createAnthropicProvider`, `createAnthropicProviderStream`, `withStreaming`, `withEnhancedStreaming`, `StreamEvent`, `classifyError`, session primitives, and the run error classes used below.
- **From the specification**: `@lostgradient/operative@0.7.0` is required to implement the [approved typed Agent API](https://github.com/stevekinney/agent-bureau/blob/0a3dcc8b169999222833901c2edb1c21b29c2249/documentation/operative-type-safe-api.md): `createAgent({ name, generate, toolbox, output?, ... })`, synchronous `RunnableAgent.run()`, non-thenable `AgentRun`, `result()`, `unwrap()`, conditional `output()`, `abort()`, disposal, and `RunResult.output`. That specification is now implemented by the published package; confirm each symbol against the installed `0.7.0` declarations rather than against the specification alone.

What has been verified directly, on 2026-09-01, is the published export map: the `./anthropic`, `./conditions`, `./durable`, `./guardrails`, `./instrumentation`, `./providers`, `./retry`, `./store`, `./streaming`, and `./test` subpaths are all present at `0.7.0`, unchanged from `0.2.0`. Individual symbol names have not been re-checked and must be confirmed against the installed declarations.

The first implementation pull request must verify the registry artifact before writing application code:

```sh
npm view @lostgradient/operative@0.7.0 version dist.integrity dist.shasum dist.tarball --json
```
...
- The browser owns the authoritative `ConversationHistory` value rendered by `@lostgradient/chat` (a `workspace:*` sibling since the 2026-08-25 monorepo merge).
...
The browser creates, renders, and stores `ConversationHistory` using the public builders re-exported by `@lostgradient/chat` (a `workspace:*` sibling since the 2026-08-25 monorepo merge). It sends `{ conversation }` to `/api/chat`. The server validates that boundary, then passes the value to `RunnableAgent.run({ conversation })` from `@lostgradient/operative@0.7.0`.
...
The exact public baseline types involved are `StreamEvent`, `RunEvent`, and `RunCompletedEvent` from `@lostgradient/operative@0.7.0`. At that version, `withEnhancedStreaming(createAnthropicProviderStream(...), { eventTarget })` supplies the structured `stream:*` events and the `AgentRun` async iterator supplies tool and run lifecycle events. `withStreaming()` alone updates the conversation but does not expose the structured streaming event target required by this wire contract.

Before CIN-436 writes application code, it must verify that the installed `@lostgradient/operative@0.7.0` declarations export those APIs and discriminants or their documented replacements. If the surface changed, this section changes first. Chatroom does not preserve both shapes with a compatibility adapter. The approved `0.7.0` Agent API specification settles the nested `RunCompletedEvent.result` shape but does not settle `StreamEvent`, `withEnhancedStreaming()`, or the tool-event vocabulary.

Export presence does not prove event _timing_. CIN-436 requires tool-call events while the provider response is still open, and that must be demonstrated against the real public wrapper at `0.7.0` rather than assumed. If `stream:tool-call-*` still arrives only after the generate function resolves, that is an upstream blocker, not permission to return to Chatroom's ad hoc `tool_call` frame.

The `0.7.0` contract nests a completed event's terminal data under `result`; the wire follows that shape instead of flattening a second result vocabulary. The route may omit unrelated public events, but it may not rename a selected discriminant or forward internal provider events. Events from the streaming target and the run iterator enter one request-local sequencer before encoding, so their relative order is explicit rather than dependent on two consumers racing to call `controller.enqueue()`.

The enhanced-streaming target must also be request-local. The published `0.7.0` `StreamEvent` union does not carry an `AgentRun` identifier, and `EventTarget` dispatch is broadcast, so a module-scoped target cannot distinguish overlapping requests. Accordingly, each request constructs its own `TypedEventTarget<StreamEventMap>`, `withEnhancedStreaming(createAnthropicProviderStream(...), { eventTarget })` wrapper, and `RunnableAgent`, while reusing the module-scoped toolbox and immutable agent configuration. If the installed declarations turn out to provide a stable public run identifier on every enhanced-streaming event, a shared target is allowed only when the listener filters on that identifier before sequencing. Arrival time, listener registration order, or conversation identity is not sufficient provenance.

> [!IMPORTANT] Decided, not yet implemented: this vocabulary requires extending the published client decoder
> The vocabulary specified above is the target contract and is settled. It is not what the browser can decode _today_: since [cinder#1472](https://github.com/stevekinney/cinder/pull/1472) the client decodes with `decodeChatStreamEvents` from `@lostgradient/chat`, whose `ChatStreamEvent` union is exactly `text | tool_call | tool_result` and which throws `'Invalid chat stream event'` on anything else. Emitting these frames against today's decoder would make every frame throw, so the decoder must be extended before the wire contract is implemented.
>
...
CIN-446 proved the server-owned model with public Operative APIs and no Bureau imports. The root `@lostgradient/operative@0.7.0` session surface is `createSessionStore()`, `createAgentSession()`, `loadAgentSession()`, `saveAgentSession()`, `createSessionHandle()`, and `resumeSession()`. Durable recovery adds `createRunEngine()`, `createCheckpointStore()`, and `createRunWorkflow()` from `@lostgradient/operative@0.7.0/durable`.

Operative deliberately does not own Chatroom's conversation-list index, so the host must maintain the index that maps a user-visible conversation to its session. On restart, the host must reconstruct workflow services through `resolveWorkflowServices`, and it must sweep orphaned `RunRef` records when a durable run can no longer be resumed. [AB-28](https://linear.app/lost-gradient/issue/AB-28), [AB-29](https://linear.app/lost-gradient/issue/AB-29), and [AB-30](https://linear.app/lost-gradient/issue/AB-30) are all Done as of 2026-08-31, and the `./durable` export subpath is present in the published `0.7.0` export map. There is no remaining upstream gate here. Downstream code still may not import Bureau internals or recreate those capabilities locally.
