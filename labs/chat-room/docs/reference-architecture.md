...
The detail route also exposes a no-store synchronization snapshot containing `id`, `title`, the full native `ConversationHistory`, and a `turnFailures` map. A failed run is recorded before its terminal wire frame: the map is keyed by the authoritative last user message from that run's result, and each value contains only the classified `kind`, `code`, `message`, and optional `retryable` fields. The session update merges metadata atomically with first-write-wins semantics and does not refresh activity, so concurrent failures and unrelated metadata survive.
