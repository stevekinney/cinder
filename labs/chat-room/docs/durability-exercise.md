...
The detail response is a no-store server snapshot. It includes the native conversation and a `turnFailures` map. When a generation fails, the server persists the classified, redacted reason against that run's last user message before emitting the terminal `run.error`; reloading therefore retains both the failed delivery marker and its controlled reason, `The assistant could not complete this turn.`. If the metadata write rejects after the run's conversation is readable, a separate local `stream:error` hint preserves the failed row through later submissions until that same authoritative id is durably confirmed, or until navigation; it does not alter GET or SSR. Provider messages, causes, stacks, response objects, and credentials never enter session metadata or the snapshot.
