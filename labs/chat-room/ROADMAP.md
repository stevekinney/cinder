...
**Both acceptance criteria are now met; found by a `contract-auditor` board-round-5 general sweep, which this section had not caught up to.** `src/routes/hydration.e2e.ts`'s `HYDRATING_ROUTES` now lists all **38** routes — `/`, `/exercises`, and all **36** exercise directories, `review-ssr-and-a11y` and `review-diff-performance` (added by `X-1`) included — counted against the filesystem rather than by adding to the previous number. The gap this item originally described (seven listed, five of them exercises, against 25 exercise directories at the time) closed somewhere in the batches since, without this section being updated to say so. (Found stale a **third** time by review on CIN-441, which added `multi-agent` as the 36th exercise — after `X-1` added a 29th and a `contract-auditor` round caught that one. The count in this line has never once been updated by the change that invalidated it, which is enough evidence to stop calling it a reminder: a new exercise route is a two-place update, `HYDRATING_ROUTES` itself and this line, and the number here is arrived at by counting `src/routes/exercises/*/` rather than by adding one. If it goes stale a fourth time the number should come out of the prose entirely and this paragraph should point at the list.)
