...
**Both acceptance criteria are now met; found by a `contract-auditor` board-round-5 general sweep, which this section had not caught up to.** `src/routes/hydration.e2e.ts`'s `HYDRATING_ROUTES` lists `/`, `/exercises`, and every directory under `src/routes/exercises/`, `review-ssr-and-a11y` and `review-diff-performance` included. The gap this item originally described (seven listed, five of them exercises, against 25 exercise directories at the time) closed somewhere in the batches since, without this section being updated to say so.

**There is deliberately no route count in this paragraph.** The previous version carried one, and it was found stale by review four separate times — `X-1` added a route, a `contract-auditor` round caught that, CIN-441 added `multi-agent`, and then review found that fixing the number here had left `ROADMAP_PROGRESS.md`'s copy of it saying something different again. Not once was it updated by the change that invalidated it. A number repeated in prose next to the list it describes is a second source of truth that only ever drifts, so the list is now the only place the count exists: to know how many routes hydrate, read `HYDRATING_ROUTES`. Adding an exercise is a one-place update again — add it to that array, which `src/routes/hydration.e2e.ts` enforces against the filesystem, and there is nothing here to keep in sync.
